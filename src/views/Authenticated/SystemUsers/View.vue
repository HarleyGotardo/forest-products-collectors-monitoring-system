...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 bg-gray-50">
    ...
    >
      ...
      <div class="px-6 sm:px-8 py-6 bg-gradient-to-r from-blue-500 to-indigo-600">
        <div class="flex items-center justify-between">
          <h2 class="text-2xl font-bold text-white">User Profile</h2>
          <!-- Avatar with Ring -->
          <div class="h-14 w-14 rounded-full bg-white p-1 ring-4 ring-white/30 shadow-lg">
            <img 
            ...
            />
            <div v-else class="h-full w-full rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center">
      ...
      <div class="p-6 sm:p-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <!-- First Name -->
          <div class="space-y-2 transition-all duration-300 p-4 rounded-xl hover:bg-gray-50">
    ...
    <div v-if="isForestProductCollector(user?.role?.id)" class="mt-12">
      ...
      <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-b border-gray-200">
          <div class="flex items-center justify-between">
            <h4 class="font-medium text-gray-700">Records for {{ user.first_name }} {{ user.last_name }}</h4>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="hidden sm:table-cell px-6 py-3.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Forest Product</th>
                <th scope="col" class="hidden sm:table-cell px-6 py-3.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date Collected</th>
                <th scope="col" class="px-6 py-3.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Cost</th>
                <th scope="col" class="hidden sm:table-cell px-6 py-3.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Processed By</th>
                <th scope="col" class="hidden sm:table-cell px-6 py-3.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Payment Status</th>
              </tr>
            </thead>
              ...
              <tr v-if="paginatedRecords.length === 0">
                <td colspan="6" class="px-6 py-12 text-center">
                  <div class="flex flex-col items-center">
                    ...
                    <p class="text-sm text-gray-400">Collection records will appear here</p>
                  </div>
                </td>
              </tr>
              <tr v-for="record in paginatedRecords" :key="record.id" class="hover:bg-blue-50 transition-colors duration-200 cursor-pointer">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600">#{{ record.id }}</td>
                <td class="hidden sm:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ record.forest_product.name }}</td>
                <td class="hidden sm:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ record.formatted_created_at }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">₱{{ record.total_cost }}</td>
                <td class="hidden sm:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ record.created_by_name }}</td>
